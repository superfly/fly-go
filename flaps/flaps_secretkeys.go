...
func (f *Client) sendRequestSecretKeys(ctx context.Context, appName, method, endpoint string, in, out any, qs url.Values, headers map[string][]string) error {
	endpoint = fmt.Sprintf("/apps/%s/secretkeys%s", url.PathEscape(appName), endpoint)
	if qs != nil {
		endpoint += "?" + qs.Encode()
	}
...
func (f *Client) ListSecretKeys(ctx context.Context, appName string, version *uint64) ([]fly.SecretKey, error) {
	...
	if err := f.sendRequestSecretKeys(ctx, appName, http.MethodGet, "", nil, &out, qs, nil); err != nil {
...
func (f *Client) GetSecretKey(ctx context.Context, appName, name string, version *uint64) (*fly.SecretKey, error) {
	...
	out := fly.SecretKey{}
	if err := f.sendRequestSecretKeys(ctx, appName, http.MethodGet, path, nil, &out, qs, nil); err != nil {
...
func (f *Client) SetSecretKey(ctx context.Context, appName, name string, typ string, value []byte) (*fly.SetSecretKeyResp, error) {
	...
	if err := f.sendRequestSecretKeys(ctx, appName, http.MethodPost, path, in, &out, nil, nil); err != nil {
...
func (f *Client) DeleteSecretKey(ctx context.Context, appName, name string) error {
	...
	path := fmt.Sprintf("/%s", url.PathEscape(name))
	if err := f.sendRequestSecretKeys(ctx, appName, http.MethodDelete, path, nil, nil, nil, nil); err != nil {
...
func (f *Client) EncryptSecretKey(ctx context.Context, appName, name string, plaintext, assoc []byte, version *uint64) (*fly.EncryptSecretKeyResp, error) {
	...
	if err := f.sendRequestSecretKeys(ctx, appName, http.MethodPost, path, in, &out, qs, nil); err != nil {
...
func (f *Client) VerifySecretKey(ctx context.Context, appName, name string, plaintext, sig []byte, version *uint64) error {
	...
	if err := f.sendRequestSecretKeys(ctx, appName, http.MethodPost, path, in, nil, qs, nil); err != nil {
